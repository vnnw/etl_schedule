create external table if not exists ods_mysql.ods_beeper_trans_settlement__trans_event_driver_expense (
    `id` bigint comment "唯一主键" ,
    `trans_event_id` bigint comment "出车记录id" ,
    `driver_id` bigint comment "计费发生的司机id" ,
    `type` int comment "价格类型 如：基础运力价格、司机提成等" ,
